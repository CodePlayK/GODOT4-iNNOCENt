...
@onready var auplayer: AudioStreamPlayer = $auplayer
@onready var dialogue_balloon: Node2D = $"../../../../../../../.."


signal spoke(letter: String, letter_index: int, speed: float)
signal paused_typing(duration: float)
signal finished_typing()
const dic_punctuation_mark:Array=["，","。","！","？","…"]
...
func type_next(delta: float, seconds_needed: float) -> void:
	...
	#根据sinc(Unicode)赋予字符音调		
	if get_parsed_text().length() >=visible_characters - 1 and dic_punctuation_mark.has(get_parsed_text()[visible_characters - 1]) or !dialogue_balloon.balloon_visiable:
		#Debug.dprint("跳过字符:%s" %get_parsed_text()[visible_characters - 1]) 
		pass
	else:
		var auplayer1=auplayer.duplicate()
		auplayer1.pitch_scale=.3+abs(sin(get_parsed_text().unicode_at(visible_characters - 1))*.7)
		#Debug.dprint("播放字符:Unicode=%s，原文:%s，音调系数：%f" %[get_parsed_text().unicode_at(visible_characters - 1),get_parsed_text()[visible_characters - 1],auplayer1.pitch_scale])
		get_tree().root.add_child(auplayer1)
		auplayer1.play()
		await auplayer1.finished
		auplayer1.queue_free()
...
func _should_auto_pause() -> bool:
	...
	if !parsed_text.length() >=visible_characters - 1:return false
	# Ignore "." if it's between two numbers
	if visible_characters > 3 and parsed_text.length() >(visible_characters - 1):
		if parsed_text[visible_characters - 1] == ".":
	...
	if visible_characters > 1 and parsed_text.length()>=visible_characters - 1 and  parsed_text[visible_characters - 1] in pause_at_characters.replace(".", "").split():
		return false
